...
export const projects = [
  ...
  },
  {
    ...
    image: '/images/projects/ngoai.svg',
  },
  {
    ...
    image: '/images/projects/inamigos.svg',
  },
  {
    ...
    image: '/images/projects/sai.svg',
  },
]

export default function FeaturedProjects() {
  return (
    <section id="featured-work" className="relative mx-auto max-w-7xl px-6 lg:px-10 py-24">
      ...
      <Reveal className="relative flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-violet">Featured Work</p>
          <h2 className="mt-3 max-w-md font-display text-3xl font-medium sm:text-4xl">
            Real builds, shipped by WebMint.
          </h2>
        </div>
        <Link to="/contact" className="group flex items-center gap-1.5 text-sm text-mute transition-colors hover:text-ink">
        ...
        {projects.map((p, i) => (
          <Reveal key={p.slug} delay={(i % 2) * 0.1}>
            <motion.div
              ...
              className="glow-border group relative flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-1"
            >
              {/* device mockup frame */}
              ...
              <div className="p-7 flex flex-col flex-1">
                ...
                <p className="mt-2 text-sm text-mute leading-relaxed flex-1">{p.description}</p>

                <div className="mt-5 flex flex-wrap gap-2">
                  {p.stack.map((s) => (
                    <span key={s} className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 font-mono text-xs text-mute-dim">
                      {s}
                    </span>
                  ))}
                </div>
